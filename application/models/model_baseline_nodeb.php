...
class Model_baseline_nodeb extends CI_Model{

	function maxdate(){
	
		 $query = $this->db->query(
		 "SELECT max(baseline_date) as date from umts_baseline.consistency_check_node_nok_hist;
;");
 return $query->result();
	}
   ...
   (SELECT mo,
           ...
           from (select nodebname_norm,cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
...
;");
 return $query->result();
   ...
   (SELECT distinct 'region'::text as type, 
            ...
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo) AS mo_total_discrepancies
           from (select region as node, nodebname_norm,cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
...
cross join (select distinct baseline_date from umts_baseline.consistency_check_node_nok_hist) test
where act = true) r 
left join 
...
cross join (select distinct baseline_date from umts_baseline.consistency_check_node_nok_hist) test
cross join (select rnc node from umts_control.cells_database where rnc = '".$node."') test2
where act = true) r 
left join 
(select baseline_date,rnc node,mo,parameter,status from umts_baseline.consistency_check_node_nok_hist CCK INNER JOIN (select distinct rnc, nodebname_norm from umts_control.cells_database) CB on cck.node = cb.nodebname_norm) c
...
;");
	 ...
	 function rnc_baseline_by_mo($reportdate,$node){
   ...
   select *, max(discrepancies) OVER (PARTITION BY type) as avg_discrepancies from
   (
   (SELECT distinct 'rnc'::text as type, 
            node,mo,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo,node) AS discrepancies,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo) AS mo_total_discrepancies
           from (select nodebname_norm,rnc as node,region,cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
...
;");
	 ...
	 function parameters_mo($reportmoname){
		 ...
		 "SELECT distinct
            ...
            from (select cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
		  from (select distinct nodebname_norm,mo,parameter from (select * from umts_control.cells_database) test1 cross join (select distinct mo,parameter from umts_baseline.consistency_check_node_nok_hist) test2) cdb
		   left join 
		   (select baseline_date, datetime, id, node, element_id, mo, parameter, baseline, configured, mml, golden, status from umts_baseline.consistency_check_node_nok_hist
          WHERE (baseline_date IN ( SELECT max(baseline_date) AS max FROM umts_baseline.consistency_check_node_nok_hist consistency_check_1))) cck 
...
;");
	 ...
	 function rnc_parameters_mo($reportmoname,$node){
		 ...
		 "SELECT distinct
            mo,parameter,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo,parameter) AS discrepancies
            from (select REGION,nodebname_norm,rnc node,cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
...
#######	 
   ...
   select *, max(discrepancies) OVER (PARTITION BY type) as avg_discrepancies from
   (
   (SELECT distinct 'region'::text as type, node as region,
            node,mo,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo,node) AS discrepancies,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo) AS mo_total_discrepancies
           from (select region as node, nodebname_norm,cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
	...
	where node = '".$node."')
	union
	(SELECT distinct 'rnc'::text as type, region,
            node,mo,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo,node) AS discrepancies,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo) AS mo_total_discrepancies
           from (select nodebname_norm,rnc as node,region,cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
...
#######
	 function region_parameters_mo($reportmoname,$node){
		 ...
		 "SELECT distinct
            mo,parameter,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo,parameter) AS discrepancies
            from (select REGION node,nodebname_norm,cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
...
where act = true) r 
left join 
(select baseline_date,REGION node,mo,parameter,status from umts_baseline.consistency_check_node_nok_hist CCK INNER JOIN (select distinct region, nodebname_norm from umts_control.cells_database) CB on cck.node = cb.nodebname_norm) c
on r.mo = c.mo and r.parameter = c.parameter and r.baseline_date = c.baseline_date and r.node = c.node
where r.node = '".$node."'
".$cond.") t
where baseline_date >= (SELECT max(baseline_date)-25 from umts_baseline.consistency_check_node_nok_hist)
...
#######	 
   ...
   SELECT distinct 'nodeb'::text as type, 
            ...
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo,node) AS discrepancies,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo) AS mo_total_discrepancies
           from (select nodebname_norm node,cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
		  from (select distinct nodebname_norm,mo,parameter from (select * from umts_control.cells_database) test1 cross join (select distinct mo,parameter from umts_baseline.consistency_check_node_nok_hist) test2) cdb
		   left join 
		   (select baseline_date, datetime, id, node, element_id, mo, parameter, baseline, configured, mml, golden, status from umts_baseline.consistency_check_node_nok_hist
          WHERE (baseline_date IN ( SELECT max(baseline_date) AS max FROM umts_baseline.consistency_check_node_nok_hist consistency_check_1))) cck 
...
where act = true) r 
left join 
(select baseline_date,nodebname_norm node,mo,parameter,status from umts_baseline.consistency_check_node_nok_hist CCK INNER JOIN (select distinct nodebname_norm from umts_control.cells_database) CB on cck.node = cb.nodebname_norm) c
on r.mo = c.mo and r.parameter = c.parameter and r.baseline_date = c.baseline_date and r.node = c.node
where r.node = '".$node."'
".$cond.") t
where baseline_date >= (SELECT max(baseline_date)-25 from umts_baseline.consistency_check_node_nok_hist)
...
#######	 
   ...
   // SELECT distinct 'nodeb'::text as type,
		  ...
		  // from (select distinct nodebname_norm,rnc,cellid,mo,parameter from (select * from umts_control.cells_database where get_nodeb_name('".$node."','cell') = nodebname_norm) test1 cross join (select distinct mo,parameter from umts_baseline.consistency_check_node_nok_hist) test2) cdb
		   // left join 
		   // (select baseline_date, datetime, id, node, element_id, mo, parameter, baseline, configured, mml, golden, status from umts_baseline.consistency_check_node_nok_hist
          // WHERE (baseline_date IN ( SELECT max(baseline_date) AS max FROM umts_baseline.consistency_check_node_nok_hist consistency_check_1))) cck 
...
// SELECT distinct 'cell'::text as type,
		  ...
		  // from (select distinct cell,rnc,cellid,mo,parameter from (select * from umts_control.cells_database where get_nodeb_name('".$node."','cell') = nodebname_norm ) test1 cross join (select distinct mo,parameter from umts_baseline.consistency_check_node_nok_hist) test2) cdb
		   // left join 
		   // (select baseline_date, datetime, id, node, element_id, mo, parameter, baseline, configured, mml, golden, status from umts_baseline.consistency_check_node_nok_hist
          // WHERE (baseline_date IN ( SELECT max(baseline_date) AS max FROM umts_baseline.consistency_check_node_nok_hist consistency_check_1))) cck 
		  // on cdb.rnc = cck.node and cdb.cellid::text = cck.element_id and cdb.mo::text = cck.mo and cdb.parameter::text = cck.parameter) t1
...
// #######
	 // function cell_parameters_mo($reportmoname,$node){
		 ...
		 // "SELECT distinct
            ...
            // sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo,parameter) AS discrepancies
            // from (select cell node,rnc,cellid,cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
		  // from (select distinct cell,rnc,cellid,mo,parameter from (select * from umts_control.cells_database where cell = '".$node."') test1 cross join (select distinct mo,parameter from umts_baseline.consistency_check_node_nok_hist) test2) cdb
		   // left join 
		   // (select baseline_date, datetime, id, node, element_id, mo, parameter, baseline, configured, mml, golden, status from umts_baseline.consistency_check_node_nok_hist
          // WHERE (baseline_date IN ( SELECT max(baseline_date) AS max FROM umts_baseline.consistency_check_node_nok_hist consistency_check_1))) cck 
		  // on cdb.rnc = cck.node and cdb.cellid::text = cck.element_id and cdb.mo::text = cck.mo and cdb.parameter::text = cck.parameter) t1
...
// where act = true) r 
// left join 
// (select baseline_date,cell node,rnc,mo,parameter,status from umts_baseline.consistency_check_node_nok_hist CCK INNER JOIN umts_control.cells_database CB on cck.node = cb.rnc and cck.element_id = cb.cellid::text) c
// on r.mo = c.mo and r.parameter = c.parameter and r.baseline_date = c.baseline_date and r.node = c.node
// where r.node = '".$node."'
// ".$cond.") t
// group by baseline_date,node order by baseline_date
...
#######	 
   ...
   select *, max(discrepancies) OVER (PARTITION BY type) as avg_discrepancies from
   (
   SELECT distinct 'cidade'::text as type,
            node,mo,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo,node) AS discrepancies,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo) AS mo_total_discrepancies
           from (select cidade node,nodebname_norm,cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
...
where act = true) r 
left join 
(select baseline_date,cidade node,mo,parameter,status from umts_baseline.consistency_check_node_nok_hist CCK INNER JOIN (select distinct cidade, nodebname_norm from umts_control.cells_database) CB on cck.node = cb.nodebname_norm) c
on r.mo = c.mo and r.parameter = c.parameter and r.baseline_date = c.baseline_date and r.node = c.node
...
#######	 
   ...
   select *, max(discrepancies) OVER (PARTITION BY type) as avg_discrepancies from
   (
   SELECT distinct 'cluster'::text as type,
            node,mo,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo,node) AS discrepancies,
            sum(case label when 'NOK' then 1 else 0 end) OVER (PARTITION BY mo) AS mo_total_discrepancies
           from (select cluster node,nodebname_norm,cdb.mo,cdb.parameter,case when status = 'NOK' then 'NOK' else 'OK' END AS label
...
where act = true) r 
left join 
(select baseline_date,cluster node,mo,parameter,status from umts_baseline.consistency_check_node_nok_hist CCK INNER JOIN (select distinct cluster, nodebname_norm from umts_control.cells_database) CB on cck.node = cb.nodebname_norm) c
on r.mo = c.mo and r.parameter = c.parameter and r.baseline_date = c.baseline_date and r.node = c.node
where r.node = '".$node."'
".$cond.") t
where baseline_date >= (SELECT max(baseline_date)-25 from umts_baseline.consistency_check_node_nok_hist)
...
// #######
	 // function uf_parameters_mo($reportmoname,$node){
		 ...
		 // "SELECT distinct
		  ...
		  // from (select distinct uf,rnc,cellid,mo,parameter from (select * from umts_control.cells_database where substring(rnc, 4, 2) = '".$node."') test1 cross join (select distinct mo,parameter from umts_baseline.consistency_check_node_nok_hist) test2) cdb
		   // left join 
		   // (select baseline_date, datetime, id, node, element_id, mo, parameter, baseline, configured, mml, golden, status from umts_baseline.consistency_check_node_nok_hist
          // WHERE (baseline_date IN ( SELECT max(baseline_date) AS max FROM umts_baseline.consistency_check_node_nok_hist consistency_check_1))) cck 
		  // on cdb.rnc = cck.node and cdb.cellid::text = cck.element_id and cdb.mo::text = cck.mo and cdb.parameter::text = cck.parameter) t1
...
// where act = true) r 
// left join 
// (select baseline_date,uf node,rnc,mo,parameter,status from umts_baseline.consistency_check_node_nok_hist CCK INNER JOIN umts_control.cells_database CB on cck.node = cb.rnc and cck.element_id = cb.cellid::text) c
// on r.mo = c.mo and r.parameter = c.parameter and r.baseline_date = c.baseline_date and r.node = c.node
// where r.node = '".$node."'
// ".$cond.") t
// group by baseline_date,node order by baseline_date
