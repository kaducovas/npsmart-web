	<?php 
		foreach($result as $row){
			...
			$csfb_prep[] = $row->csfbprep;
			$fails_csfb_prep[] = $row->fails_csfb_prep;
		...
		$fails_csfb_prep = array_map("tonull", $fails_csfb_prep);
...
///////////////////////////////////////////////////////////rrc/////////////////////////////////////////////	
		var csfb_prep = new Highcharts.Chart({
		chart: {
				renderTo: 'wcchart1',
				alignTicks:false,
				...
				tooltip: {
					...
					///	showInLegend: false,
						name: 'CSFB PREP',
						data: [<?php echo join($csfb_prep, ',') ?>]///[7.0, 6.9, 9.5, 14.5, 18.2, 21.5, 25.2, 26.5, 23.3, 18.3, 13.9, 9.6]
						///data: JSON.parse("[" + acc_rrc + "]")
					},{
			            name: 'CSFB Prep Fails',
						type: 'column',
						...
						data: [<?php echo join($fails_csfb_prep, ',') ?>]
			        }
					]							
		});		
