	<?php 
		foreach($result as $row){
			###Such a condition was needed due to the fact both retention worst cells for nqi and mobility have the same names.
			if ($kpi_family == "nqi"){
			$node = $row->node;
			$datetime[] = $row->date;	
			$retention_cs_succ_rate[] = $row->retention_cs;			
			$retention_ps_succ_rate[] = $row->retention_ps;
			} else {
			$node = $row->node;
		...
		chart: {
				...
				tooltip: {
					...
					///	showInLegend: false,
						...
						data: [<?php echo join($retention_cs_succ_rate, ',') ?>]///[7.0, 6.9, 9.5, 14.5, 18.2, 21.5, 25.2, 26.5, 23.3, 18.3, 13.9, 9.6]
